...
export class UpdateUserProfileCommand {
    ...
    async execute(uid: string, dto: UpdateUserProfileDto): Promise<any> {
        ...
        if (!policy.canWriteSelf()) {
            throw new ForbiddenException('User status does not permit profile updates');
        }
        ...
        if (dto.firstName || dto.lastName) {
            try {
                const firstName = dto.firstName ? dto.firstName.trim() : aggregate.profile!.firstName;
                const lastName = dto.lastName ? dto.lastName.trim() : aggregate.profile!.lastName;
