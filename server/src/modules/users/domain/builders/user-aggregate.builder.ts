...
export class UserAggregateBuilder {
    ...
    withFirebaseRecord(record: admin.auth.UserRecord): this {
        this.identity = new UserIdentity({
            ...
            creationTimestamp: record.metadata.creationTime ? new Date(record.metadata.creationTime) : null,
            lastRefreshTimestamp: record.tokensValidAfterTime ? new Date(record.tokensValidAfterTime) : null,
            lastSignInTimestamp: record.metadata.lastSignInTime ? new Date(record.metadata.lastSignInTime) : null,
        });
        return this;
    }
    ...
    build(): UserAggregate {
        if (!this.identity) {
            throw new Error('UserIdentity (Firebase Record) is required to build a UserAggregate');
        }
