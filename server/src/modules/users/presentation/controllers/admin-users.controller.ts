import { Controller, Get, Patch, Delete, Body, Param, Req, HttpCode, HttpStatus, UseGuards, Query } from '@nestjs/common';
...
export class AdminUsersController {
    ...
    async listUsers(
        ...
        const options = { status, role, limit: limit ? Number(limit) : undefined, cursor };
    ...
    @Patch(':uid')
    @ApiOperation({ summary: 'Admin specific update to a user profile (e.g., set status)' })
    ...
    async softDeleteUser(@Req() request: any, @Param('uid') targetUid: string, @Body() body: any) {
        const callerUid = request.user.uid;
        await this.softDeleteUserCommand.execute(targetUid, callerUid, true, { reason: body?.reason });
    }

    @Delete(':uid/permanent')
    @HttpCode(HttpStatus.NO_CONTENT)
    @ApiOperation({ summary: 'Admin permanently delete a user (scrub)' })
    async permanentDeleteUser(@Req() request: any, @Param('uid') targetUid: string, @Body() body: any) {
        const callerUid = request.user.uid;
        const alsoDeleteProfile = body?.alsoDeleteProfile !== false;
        await this.permanentDeleteUserCommand.execute(targetUid, callerUid, { reason: body?.reason, alsoDeleteProfile });
    }
}
