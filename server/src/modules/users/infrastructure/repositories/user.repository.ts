...
export class UserRepository implements IUserRepository {
  ...
  async saveProfile(aggregate: UserAggregate): Promise<void> {
    ...
    const firestoreData = {
      ...
      deleted_at: data.deleted_at
        ? admin.firestore.Timestamp.fromDate(data.deleted_at)
        : null,
    };
  ...
  async softDelete(
    ...
    try {
      ...
      this.logger.error(
        `Failed to disable Firebase Auth for soft-deleted user ${uid}`,
        error,
      );
    }
  }
  ...
  async list(options: {
    ...
    let query: admin.firestore.Query =
      this.firebaseService.firestore.collection(this.PROFILES_COLLECTION);
    ...
    if (options.cursor) {
      const cursorDoc = await this.firebaseService.firestore
        .collection(this.PROFILES_COLLECTION)
        .doc(options.cursor)
        .get();
      if (cursorDoc.exists) {
