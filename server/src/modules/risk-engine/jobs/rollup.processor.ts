...
export class RollupProcessor extends WorkerHost {
    ...
    private async handleHourlyRollup() {
        ...
        for (const cell of cells) {
            ...
            const spikeNorm = this.spikeService.computeSpikeNorm(cell.last_24h, cell.baseline_hour_avg);
            ...
            // Update level
            if (cell.risk_score >= 7) cell.risk_level = (cell as any).constructor.RiskLevel?.HIGH || 'high';
            else if (cell.risk_score >= 4) cell.risk_level = (cell as any).constructor.RiskLevel?.MEDIUM || 'medium';
