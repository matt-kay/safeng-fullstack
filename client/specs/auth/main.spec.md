# Login and Profile Setup Specification

## Overview
This specification details the authentication flow utilizing phone number sign-in via Firebase Auth, and the subsequent profile setup for new users in the SafeMe client application.

## Setup Prerequisites
Platform-specific Firebase Project setup is required before implementing this flow. Refer to the individual platform specifications for exact details (`login-profile.ios.spec.md` and `login-profile.android.spec.md`). 

**Note for Local Development:** Both clients must be configured to connect to the Firebase Auth Emulator on port 9099 during debug.
